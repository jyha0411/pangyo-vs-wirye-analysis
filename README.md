...
## 프로젝트 개요
판교테크노밸리(성공 사례)와 위례신도시(미성숙 업무단지)를 대상으로
...
## 데이터 출처 및 기준 시점
- **건축물 용도 (판교)**: OpenStreetMap (OSM) 건물 폴리곤 + 국토교통부 건축데이터민간개방시스템 건축물대장 (2026년 6월 기준)
- **건축물 용도 (위례)**: 국토교통부 건축데이터민간개방시스템 건축물대장 (송파구·수정구·하남시, 2026년 6월 기준)
- **집계구 단위 인구 및 종사자 수**: SGIS 통계지리정보서비스 (2023~2024년 기준)
- **집계구 경계**: SGIS 통계지리정보서비스 (2025년 2분기 기준)
- **도로망 및 건물 폴리곤**: OpenStreetMap (OSM)
- **수도권 지하철 네트워크**: 강의실 LMS 배포 자료 (2026년 5월 기준)
...
3. **교통망 및 등시간권 분석** (`04_subway.py`, `06_isochrone_polygon.py`, `09_station_area.py`)  
   지하철 네트워크 그래프 기반 Dijkstra 최단경로 알고리즘으로 30분/60분 등시간권 폴리곤 생성 및 역세권 면적 비율(500m/1km) 산출

4. **토지이용 및 건축물 데이터 가공** (`05_buildings.py`, `07_buildings_geojson.py`, `08_geocoding.py`)  
   OSM 건물 폴리곤과 건축물대장을 결합하여 용도별 GeoJSON 생성. 위례 건축물은 좌표 미제공으로 구역 경계 내 격자 배치 방식 적용(한계점으로 명시)
